<?php
class SecondaryOffice
{
    private $wpdb, $long_id = 0, $cont_id = 0;
    public $tableLatLong = "ics_lat_long";
    public $tableSecondaryOffice = "ics_secondary_office";
    ...
    function createDB()
    {
        ...
        ";
        $sql2 = "
            CREATE TABLE IF NOT EXISTS `$this->tableSecondaryOffice` (
              `id` INT (11) UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
              `title` VARCHAR (255) NULL,
              `description` TEXT NULL,
              `latlong` TEXT NULL,
...
//        dbDelta($sql1);
    ...
    public function getAlldataSecondaryOffice($status = 'ALL')
    {
        $this->createDB();
        $wherestatus = '';
        if ($status != 'ALL') {
        ...
        $myresult = $this->wpdb->get_results("SELECT * FROM $this->tableSecondaryOffice{$wherestatus}");
        return $myresult;
    }
    ...
    {
        if (!$this->cont_id) {
            $this->cont_id = $this->wpdb->get_var("select count(id) from $this->tableSecondaryOffice");
        }
        return $this->cont_id ? $this->cont_id : 0;
    }

    public function secondaryOfficeUpdate()
    {
        ...
        $latitude = true; /*Array ( [title] => Array ( [0] => ) [latitude] => Array ( [0] => ) [desc] => Array ( [0] => ) [imgpath] => Array ( [0] => ) [linkurl] => Array ( [0] => ) [titlelo] => Array ( [0] => ) [desclo] => Array ( [0] => ) [addesslo] => Array ( [0] => ) [phonelo] => Array ( [0] => ) [faxlo] => Array ( [0] => ) [emaillo] => Array ( [0] => ) [imagelo] => Array ( [0] => ) [gallery] => Save ) */
        if ($this->getcountContaceAll() < $insertcount && $this->getcountContaceAll() != 0) {
            $cindex = 0;
            for ($i = 0; $i < $this->getcountContaceAll(); $i++) {
                if ($latitude) {
                    $this->updateSecondaryOffice($i + 1, $title[$i], $desc[$i], $addess[$i], $phone[$i], $imgpath[$i], $fax[$i], $email[$i], 'publish', $latlong[$i]);
                    $cindex++;
                }
            }
            for ($i = $cindex; $i < $insertcount; $i++) {
                if ($latitude) {
                    $this->addSecondaryOffice(
                        $title[$i],
                        $desc[$i],
                        $addess[$i],
                        $phone[$i],
            ...
            for ($i = $cindex; $i < $this->getcountContaceAll(); $i++) {
                $this->updateSecondaryOffice($i + 1, '', '', '', '', '', '', '', 'draft', $latlong[$i]);
            }
...
link create_datetime update_datetime publish latitude*/
    ...
    public function addSecondaryOffice($title = '', $desc = '', $address = '', $phone = '', $img = '', $fax = '', $email = '', $publish = 'publish', $latlong= '')
    { /*id,title,description,address,phone,fax,email,image,create_datetime
update_datetime,publish
*/
        return $this->wpdb->query(
            "INSERT INTO  $this->tableSecondaryOffice
(title,description,address,phone,fax,email,image,create_datetime,update_datetime,publish,latlong)
					VALUES (
					'{$title}','{$desc}','{$address}','{$phone}','{$fax}','{$email}','{$img}',NOW(),NOW(),'{$publish}','{$latlong}')");
    }

    public function updateSecondaryOffice(
        $cid,
        $title = '',
        $desc = '',
        $address = '',
