...
if (!class_exists('ImageGallery')) {
    include_once("class/ClassImageGallery.php");
    global $wpdb;
...
if ($postPage == 'image_gallery') {
    if ($typePost == 'add') {
        ...
        if (is_user_logged_in()) {
            ...
            if ($classImageGallery->addData($gtitle, $glink, $gsort, $pathimg, $gdesc)) {
        ...
        exit();
    } else if ($typePost == 'json_image_gallery') {
...
//$p = new pagination();
        ...
        $classImageGallery->classPagination->target(network_site_url('/') . "?tabPage=tab1&typePost=json_image_gallery");
...
function theme_image_gallery_page()
{
    ...
    <script type="text/javascript" src="<?php bloginfo('template_directory'); ?>/lib/js/image_gallery.js"></script>
